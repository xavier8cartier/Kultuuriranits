...
import { defaultBookings, programs } from '@/lib/mockData';
...
export default function MinuBroneeringudPage() {
  ...
  const submitFeedback = () => {
    ...
    if (!authorName.trim()) {
      showToast('Palun sisesta oma nimi.');
      return;
    }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
      ...
      <div className="flex items-center gap-2 mb-8 bg-gray-100 p-1.5 rounded-full w-fit">
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === 'koik' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
        >
          Kõik
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === 'tulevased' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
        >
          Tulevased
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === 'moodunud' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
        >
          Möödunud
        </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl border border-gray-150 w-full max-w-lg overflow-hidden animate-slide-in">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Sinu nimi</label>
                <input
                  type="text"
                  value={authorName}
